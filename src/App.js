...
const fetchOffersList = (parameters) => {
	return fetch(`https://e6di35qzm7.execute-api.us-west-2.amazonaws.com/latest/directory?${parameters || ''}`)
	.then(res => res.json())
	.then(({data}) => data)
};
...
export default function App() {
	...
	useEffect(() => {
		...
		window.document.getElementById('directory-list').onscroll = (scroll) => { onScrollY(scroll) };
	},[])

	const onScrollY = (scroll) => {
		const scrollY = scroll.currentTarget.scrollTop;
		const scrollHeight = window.document.getElementById('directory-list').scrollHeight;
		const offsetHeight = window.document.getElementById('directory-list').offsetHeight;

		if (scrollY + offsetHeight >= scrollHeight - 10 && myCounter.current < parseInt(scrollHeight / (offsetHeight - 65))) {
			setCounter(myCounter.current + 1);
			setIsLoading(true);
			fetchOffersList(`epp=18&p=${myCounter.current + 1}`)
			.then((Offers) => {
				setIsLoading(false);
	...
	const searchOffer = (offer) => {
		setIsLoading(true);
		fetchOffersList(`epp=18&p=1&name=${offer}`)
		.then(data => {
			setOffers(data);
			setFilter('');
			setOrder('');
	...
	return (
		<>
			<FilterBar onSearch={searchOffer} onSetOrder={orderBy} order={order} filter={filter}></FilterBar>
